...
def do_train(
    ...
    for iteration, (images, targets, _) in enumerate(data_loader, start_iter):
        ...
        if iteration % 20 == 0 or iteration == max_iter:
            ...
            print('position 1')
            print(str(meters).split('  ')[0].split(':')[1].split(' ')[1])

            temp_dict['loss'].append(str(meters).split('  ')[0].split(':')[1].split(' ')[1])
            temp_dict['loss_box_reg'].append(str(meters).split('  ')[1].split(':')[1].split(' ')[1])
            temp_dict['loss_classifier'].append(str(meters).split('  ')[2].split(':')[1].split(' ')[1])
            temp_dict['loss_mask'].append(str(meters).split('  ')[3].split(':')[1].split(' ')[1])
            temp_dict['loss_objectness'].append(str(meters).split('  ')[4].split(':')[1].split(' ')[1])
            temp_dict['loss_rpn_box_reg'].append(str(meters).split('  ')[5].split(':')[1].split(' ')[1])
            temp_dict['time'].append(str(meters).split('  ')[6].split(':')[1].split(' ')[1])
